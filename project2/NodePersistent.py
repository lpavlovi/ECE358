...
class Node():
    def __init__(self, L, W, A, _id, tick_length, p_persistent, Medium):
        """
        Inputs:
        ...
        self.W = W
        self.A = A
        self.p_persistent = p_persistent
        self.q = deque()
        self.Medium = Medium
        ...
        # The waiting duration is initialized to Transmission Duration + propagation delay
        self.waiting_duration = self.transmission_duration + 10
    ...
    def setBackoffState(self, current_tick):
        ...
        new_backoff_duration = self.calculateBEB(self.i)
    ...
    # NOTE: Might not be necessary since probability does not require any ticks
    def setProbabilityState(self, current_tick):
        self.current_state = NodeState.PROBABILITY
        self.intermediate_milestone = int(current_tick + 1)
        self.servicing_milestone = self.intermediate_milestone
    ...
    def setWaitingState(self, current_tick):
        ...
        self.servicing_milestone = current_tick + self.waiting_duration
        loggingModule.stateChanged(self.id, NodeState.WAITING, current_tick)
    ...
    def shouldWeTransmitNow(self):
        u = random.uniform(0,1)
        # If the random number is less than or equal to the probability
        # then we should transmit now
        if u <= self.p_persistent:
            return True

        # otherwise we should not transmit now
        else:
            return False
    ...
    def servicing(self, current_tick):
        ...
        if self.current_state == NodeState.SENSING:
            # If the medium is busy - start sensing again
            if self.isMediumBusy(current_tick):
                self.setSensingState(current_tick)

            # If the medium is available for the sensing duration
            # Check probability of transmission - don't transmit yet
            elif self.servicing_milestone == current_tick:
                # TODO: Check probability
                # TODO: Implement Probability State
                #self.setProbabilityState(current_tick)
                # Should we transmit
                if self.shouldWeTransmitNow():
                    # YES
                    # Start transmitting
                    # Don't check the medium here since we already know that it is
                    # not busy at this tick
                    self.setTransmititngState(current_tick)
                else:
                    # NO
                    # Wait a long while
                    # TODO: Update waiting state to go to probability state
        ...
        # WAITING
            ...
            if self.servicing_milestone == current_tick:
                # TODO: Goes to probability check instead of sensing state
                #self.setSensingState(current_tick)
                #self.intermediate_milestone = int(current_tick + 1)
                if self.shouldWeTransmitNow():
                    # YES
                    # Start transmitting
                    # -- actually check the medium again
                    # If the medium is not busy -----> Transmit
                    if self.isMediumBusy(current_tick):
                        self.setTransmititngState(current_tick)

                    # If the medium is busy ---------> BEB
                    else:
                        self.setBackoffState(current_tick)
                else:
                    # NO
